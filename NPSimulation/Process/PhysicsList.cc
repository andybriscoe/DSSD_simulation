...
void PhysicsList::ReadConfiguration(std::string filename){
  ...
  while(file >> name >> str_value){
    ...
    if(name == "EmPhysicsList")
      ...
      m_Menate_R = value;
    else if (name == "Trapped_Electrons")
      m_Trapped_Electrons = value;
    else
...
void PhysicsList::AddParametrisation(){
  ...
  while ((*theParticleIterator)()){
    ...
    if(m_Trapped_Electrons > 0 &&  (name == "e-" || type == "nucleus") ){
    //if(m_Trapped_Electrons > 0) {
      Trapped_Electrons* theTrappedElectrons = new Trapped_Electrons("Trapped_electrons_electron");
      pmanager->AddDiscreteProcess(theTrappedElectrons);
      std::cout <<"||--------------------------------------------------||" << std::endl;
      std::cout <<"  Trapped electron catcher added to Process Manager   " << std::endl;
      std::cout <<"||--------------------------------------------------||" << std::endl;
    }
  }
}
