...
G4bool PS_Silicon_Rectangle::ProcessHits(G4Step* aStep, G4TouchableHistory*){
    ...
    //WHen simulating one pixel, the x and y sides were automatically summed -> I didn't want this for number of gamma rays, only 1 side, so I comment out the line below and it looks good. It would be nicer to have x-side, y-side and then sum.. should take care with turning this back on when needed:Andy 131222
