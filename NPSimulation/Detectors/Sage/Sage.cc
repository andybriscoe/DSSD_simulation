...
namespace Sage_NS
{
	...
	const double ResoEnergy = 2.1*keV ;
    ...
    //const vector<double> deadWidth = {1.0*um, 0.6*um, 3.5*um, 1.5*um};
    const vector<double> deadWidth = {0.6*um, 3.5*um, 1.5*um, 1.5*um};
    const vector<double> sourceWidth = {6.4*um, 1.7*um};
    //const vector<double> deadWidth = {0.*um, 0.*um, 3.5*um, 1.5*um};// standard!!!
    //const vector<double> deadWidth = {1.0*um, 0.6*um, 3.5*um, 1.5*um};
...
Sage::Sage()
{
	...
	m_SageScorer = 0;
	m_SiliconDetector = 0;
	m_SiliconDeadLayer1 = 0;
	m_SiliconDeadLayer2 = 0;
	m_SiliconDeadLayer3 = 0;
	m_SiliconDeadLayer4 = 0;
...
G4LogicalVolume* Sage::BuildSiliconDetector()
{
	if(!m_SiliconDetector)
	{
		...
		G4Material* DeadLayerMaterial1 = MaterialManager::getInstance()->GetMaterialFromLibrary("SiO2");
		G4Material* DeadLayerMaterial2 = MaterialManager::getInstance()->GetMaterialFromLibrary("Al");

        G4Tubs* deadLayer1 = new G4Tubs("SageSiliconDeadLayer1", 0, 24.*mm, Sage_NS::deadWidth[0]*0.5,0,360*deg);
        //m_SiliconDeadLayer1 = new G4LogicalVolume(deadLayer1, DeadLayerMaterial1, "logic_Sage_silicon_dead_layer1",0,0,0);//standard
        m_SiliconDeadLayer1 = new G4LogicalVolume(deadLayer1, DeadLayerMaterial2, "logic_Sage_silicon_dead_layer1",0,0,0);
        m_SiliconDeadLayer1->SetVisAttributes(G4VisAttributes(G4Colour(0.5, 0, 0)));

        G4Tubs* deadLayer2 = new G4Tubs("SageSiliconDeadLayer2", 0, 24.*mm, Sage_NS::deadWidth[1]*0.5,0,360*deg);
        //m_SiliconDeadLayer2 = new G4LogicalVolume(deadLayer2, DeadLayerMaterial2, "logic_Sage_silicon_dead_layer2",0,0,0);//standard
        m_SiliconDeadLayer2 = new G4LogicalVolume(deadLayer2, DeadLayerMaterial1, "logic_Sage_silicon_dead_layer2",0,0,0);
        m_SiliconDeadLayer2->SetVisAttributes(G4VisAttributes(G4Colour(0.5, 0.5, 0)));

        G4Tubs* deadLayer3 = new G4Tubs("SageSiliconDeadLayer3", 0, 24.*mm, Sage_NS::deadWidth[2]*0.5,0,360*deg);
        //m_SiliconDeadLayer3 = new G4LogicalVolume(deadLayer3, DeadLayerMaterial1, "logic_Sage_silicon_dead_layer3",0,0,0);//standard
        m_SiliconDeadLayer3 = new G4LogicalVolume(deadLayer3, DeadLayerMaterial2, "logic_Sage_silicon_dead_layer3",0,0,0);
        m_SiliconDeadLayer3->SetVisAttributes(G4VisAttributes(G4Colour(0.5, 0, 0)));

        G4Tubs* deadLayer4 = new G4Tubs("SageSiliconDeadLayer4", 0, 24.*mm, Sage_NS::deadWidth[3]*0.5,0,360*deg);
        m_SiliconDeadLayer4 = new G4LogicalVolume(deadLayer4, DeadLayerMaterial2, "logic_Sage_silicon_dead_layer4",0,0,0);
        m_SiliconDeadLayer4->SetVisAttributes(G4VisAttributes(G4Colour(0.5, 0.5, 0)));
...
void Sage::ConstructChamber(G4LogicalVolume* world)
{
	...
	G4Tubs* TargetBacking = new G4Tubs("TargetBacking", 0., 26*mm/2, Sage_NS::sourceWidth[0]/2.,0.,360.*deg);
	G4Tubs* TargetFront = new G4Tubs("TargetFront", 0., 26*mm/2, Sage_NS::sourceWidth[1]/2, 0., 360.*deg);
...
//			false, 0, Overlap);
    ...
    if (SourceGeometry==1)
    {
        G4cout << "Source geometry in" << endl;

        new G4PVPlacement(0,G4ThreeVector(0.,0.,-Sage_NS::sourceWidth[0]/2),
                TargetBacking_log,
                "Source_backing",
                world,
                false, 0, Overlap);

        new G4PVPlacement(0,G4ThreeVector(0.,0.,Sage_NS::sourceWidth[1]/2),
                TargetFront_log,
                "Source_Front",
                world,
                false, 0, Overlap);
...
void Sage::ReadConfiguration(NPL::InputParser parser)
{
	...
	vector<string> foil = {"CarbonFoil","InnerDiameter","SiThickness", "SourceGeometry"};
	...
	for(unsigned int i = 0 ; i < blocks.size() ; i++){
		if(blocks[i]->HasTokenList(cart)){
			...
			SourceGeometry = blocks[i]->GetInt("SourceGeometry");
...
void Sage::ConstructDetector(G4LogicalVolume* world)
{
	...
	for (unsigned short i = 0 ; i < m_R.size() ; i++)
	{
		...
		G4ThreeVector Det_Dead_Layer_pos1 = Det_pos - Det_pos.unit()*(SiThickness*0.5+Sage_NS::deadWidth[0]*0.5);
		G4ThreeVector Det_Dead_Layer_pos2 = Det_Dead_Layer_pos1 - Det_pos.unit()*(Sage_NS::deadWidth[0]*0.5+Sage_NS::deadWidth[1]*0.5);
		G4ThreeVector Det_Dead_Layer_pos3 = Det_Dead_Layer_pos2 - Det_pos.unit()*(Sage_NS::deadWidth[1]*0.5+Sage_NS::deadWidth[2]*0.5);
		G4ThreeVector Det_Dead_Layer_pos4 = Det_Dead_Layer_pos3 - Det_pos.unit()*(Sage_NS::deadWidth[2]*0.5+Sage_NS::deadWidth[3]*0.5);
		...
		if(m_Shape[i] == "Silicon")
		{
            ...
            new G4PVPlacement(G4Transform3D(G4RotationMatrix(0.,0.,0.),Det_Dead_Layer_pos1),
                    m_SiliconDeadLayer1,"Sage_Dead_Layer1", world, false, i+1);
            new G4PVPlacement(G4Transform3D(G4RotationMatrix(0.,0.,0.),Det_Dead_Layer_pos2),
                    m_SiliconDeadLayer2,"Sage_Dead_Layer2", world, false, i+1);
            new G4PVPlacement(G4Transform3D(G4RotationMatrix(0.,0.,0.),Det_Dead_Layer_pos3),
                    m_SiliconDeadLayer3,"Sage_Dead_Layer3", world, false, i+1);
            //new G4PVPlacement(G4Transform3D(G4RotationMatrix(0.,0.,0.),Det_Dead_Layer_pos4),
            //        m_SiliconDeadLayer4,"Sage_Dead_Layer4", world, false, i+1);
		}
...
//cout << "Scorer started\n";
	...
	for (Pixel_itr = PixelHitMap->GetMap()->begin() ; Pixel_itr != PixelHitMap->GetMap()->end() ; Pixel_itr++)
	{
		...
		if(Energy>Sage_NS::EnergyThreshold){
            ...
            if (PixelNbr < 90) //sometimes this is picking up weird numbers...
            {
