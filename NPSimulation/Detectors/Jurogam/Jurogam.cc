...
void Jurogam::ReadSensitive(const G4Event* event)
{
    ...
    G4int HPGECollectionID = G4SDManager::GetSDMpointer()->GetCollectionID("JurogamScorer/Ge");
    if(HPGECollectionID == -1) {
        G4cerr << "ERROR: No Collection found for GeScorer: Skipping processing of HPGe Hit" << G4endl;
        return;
    }
    ...
    for (HPGE_itr = HPGEHitMap->GetMap()->begin() ; HPGE_itr != HPGEHitMap->GetMap()->end() ; HPGE_itr++){
        ...
        if(Energy>Jurogam_NS::EnergyThreshold)
        {
            ...
            int GeNbr = (int) Info[2];
            int CrystalNbr = Info[3];
            m_Event->SetEnergy(GeNbr,Energy);
            m_Event->SetTime(GeNbr,Time); 
            m_Event->SetCrystal(GeNbr,CrystalNbr+1);
        }
    ...
    ///////////
    // BGO
    NPS::HitsMap<G4double*>*     BGOHitMap;
    std::map<G4int, G4double**>::iterator    BGO_itr;

    G4int BGOCollectionID = G4SDManager::GetSDMpointer()->GetCollectionID("JurogamBGOScorer/GeBGO");
    if(BGOCollectionID == -1) {
        G4cerr << "ERROR: No Collection found for GeBGOScorer: Skipping processing of BGO Hit" << G4endl;
        return;
    }
    BGOHitMap = (NPS::HitsMap<G4double*>*)(event->GetHCofThisEvent()->GetHC(BGOCollectionID));

    // Loop on the BGO map
    for (BGO_itr = BGOHitMap->GetMap()->begin() ; BGO_itr != BGOHitMap->GetMap()->end() ; BGO_itr++){

        G4double* Info = *(BGO_itr->second);
        ...
        if(Energy>Jurogam_NS::BGOEnergyThreshold)
        {
            ...
            int GeNbr = (int) Info[2];
            m_Event->Set_BGO_Energy(GeNbr,Energy);
            m_Event->Set_BGO_Time(GeNbr,Time); 
        }
        //m_Event->SetCrystal(GeNbr,CrystalNbr+1);
    }
    ...
    // clear map for next event
    BGOHitMap->clear();
    //m_JurogamScorer->clear();
    //m_JurogamBGOScorer->clear();
}
//void Jurogam::ReadSensitive(const G4Event* event)
//{
//    //CalorimeterScorers::PS_Calorimeter* Scorer= (CalorimeterScorers::PS_Calorimeter*) m_JurogamScorer->GetPrimitive(0);
//
//    //unsigned int size = Scorer->GetMult(); 
//    //for(unsigned int i = 0 ; i < size ; i++){
//    //    double Energy = RandGauss::shoot(Scorer->GetEnergy(i),Jurogam_NS::ResoEnergy);
//    //    if(Energy>Jurogam_NS::EnergyThreshold){
//    //        double Time = RandGauss::shoot(Scorer->GetTime(i),Jurogam_NS::ResoTime);
//    //        int DetectorNbr = Scorer->GetLevel(i)[0];
//    //        m_Event->SetEnergy(DetectorNbr,Energy);
//    //        m_Event->SetTime(DetectorNbr,Time); 
//    //    }
//    //}
//    //CalorimeterScorers::PS_Calorimeter* ScorerBGO= (CalorimeterScorers::PS_Calorimeter*) m_JurogamBGOScorer->GetPrimitive(0);
//
//    //unsigned int sizeBGO = ScorerBGO->GetMult(); 
//    //for(unsigned int i = 0 ; i < sizeBGO ; i++){
//    //    double Energy = RandGauss::shoot(ScorerBGO->GetEnergy(i),Jurogam_NS::BGOResoEnergy);
//    //    if(Energy>Jurogam_NS::EnergyThreshold){
//    //        double Time = RandGauss::shoot(ScorerBGO->GetTime(i),Jurogam_NS::BGOResoTime);
//    //        int DetectorNbr = ScorerBGO->GetLevel(i)[0];
//    //        m_Event->SetEnergy(DetectorNbr,Energy);
//    //        m_Event->SetTime(DetectorNbr,Time); 
//    //    }
//    //}
...
{ 
	...
	G4VPrimitiveScorer* JurogamScorer = new GESCORERS::PS_Ge("Ge",level, 0) ;
	//// BGO Shields
	vector<int> levelBGO; levelBGO.push_back(1);
	G4VPrimitiveScorer* JurogamBGOScorer = new GESCORERS::PS_Ge("GeBGO",levelBGO, 0) ;
    //G4VPrimitiveScorer* JurogamScorer = new  GESCORERS::PS_Ge("Ge",0);
    //G4VPrimitiveScorer* JurogamBGOScorer = new  GESCORERS::PS_Ge("GeBGO",0);
	...
	m_JurogamScorer->RegisterPrimitive(JurogamScorer);
	m_JurogamBGOScorer->RegisterPrimitive(JurogamBGOScorer);
